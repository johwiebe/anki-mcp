...
@pytest.mark.asyncio
async def test_get_review_stats_with_deck_name(monkeypatch):
    """Test review statistics with deck filtering (shows limitation message)."""
    deck_stats = [{
        "deck_id": 1,
        "name": "TestDeck",
        "total_in_deck": 100,
    ...
    review_data = [["2024-01-01", 50]]

    async def mock_anki_request(action, **kwargs):
        if action == "getDeckStats":
            assert kwargs["decks"] == ["TestDeck"]
            return {"success": True, "result": deck_stats}
        elif action == "getNumCardsReviewedByDay":
            return {"success": True, "result": review_data}
        return {"success": False, "error": "Unexpected action"}

    monkeypatch.setattr("anki_mcp.tools.get_review_stats.make_anki_request", mock_anki_request)

    result = await get_review_stats(deck_name="TestDeck")

    text = result[0].text
    assert "Deck 'TestDeck' current statistics:" in text
    assert "Total cards: 100" in text
    assert "Reviews today: 15" in text
    assert "Per-day review history by deck is not available" in text


@pytest.mark.asyncio
async def test_get_review_stats_deck_not_found(monkeypatch):
    """Test handling when specified deck is not found."""
    async def mock_anki_request(action, **kwargs):
        if action == "getDeckStats":
            return {"success": True, "result": []}  # Empty result means deck not found
        return {"success": False, "error": "Unexpected action"}

    monkeypatch.setattr("anki_mcp.tools.get_review_stats.make_anki_request", mock_anki_request)

    result = await get_review_stats(deck_name="NonExistentDeck")

    text = result[0].text
    assert "Deck 'NonExistentDeck' not found" in text


@pytest.mark.asyncio
async def test_get_review_stats_deck_api_failure(monkeypatch):
    """Test handling when deck stats API fails."""
    async def mock_anki_request(action, **kwargs):
        if action == "getDeckStats":
    ...
    text = result[0].text
    assert "Failed to retrieve deck statistics: Database error" in text
