...
from anki_mcp.tools.add_or_update_notes import (
    Note,
    add_or_update_notes,
    update_note,
    add_note,
...
async def test_add_note_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "anki_mcp.tools.add_or_update_notes.make_anki_request", mock_anki_request
    )
...
async def test_add_or_update_notes_all_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "anki_mcp.tools.add_or_update_notes.update_note", mock_update_note
    )
...
async def test_add_or_update_notes_all_failure(monkeypatch):
    ...
    assert (
        "Failed to update note 'Failed Note 2' with ID 9999: Note not found"
        in text_content
    )
