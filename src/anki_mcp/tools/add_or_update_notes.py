...
from anki_mcp.tools.utils import (
    DEFAULT_DECK_NAME,
    DEFAULT_MODEL_NAME,
    make_anki_request,
)
...
class Note(BaseModel):
    ...
    deckName: Annotated[
        str, Field(description="Deck name (optional)", default=DEFAULT_DECK_NAME)
    ]
    modelName: Annotated[
        str, Field(description="Model name (optional)", default=DEFAULT_MODEL_NAME)
    ]
    fields: Annotated[
        Dict[str, str], Field(description="Field values for the note (varies by model)")
    ]
    tags: Annotated[
        Optional[List[str]],
        Field(description="Tags to assign to the note (optional)", default=None),
    ]
